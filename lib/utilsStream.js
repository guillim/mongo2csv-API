...
const goThroughStream = async function(inputPath,functionToExecute,outsideArg){
  console.log('Entering - ', functionToExecute.name);
...
const getSellerASINlink = async function(results,req,db,jsonFullPath){
  try {
...
const fullProcess = async function(results,post,keywords,utils_positionToCTRs,message,res,req,db){
  ...
  let jsoniniFullPath = './app/tmp/' + new Date().getTime() + '.jsonini'
  let jsonFullPath = jsoniniFullPath.replace(/jsonini/,'json')

  utils.writeAFile(results,jsoniniFullPath)

	let sellerASIN = await getSellerASINlink(results,req,db,jsoniniFullPath)
