...
const fullProcess = async function(results,post,keywords,utils_positionToCTRs,message,res,req,db){
	...
	console.log('sellerASIN at ', (new Date().getTime() - debut) / 1000  );
	results.map( (result) => {
		let keyword = keywords.find( (el) => { return el.keyword === result.c01_keyword;  });
    if (!result.c02_marketplaceName || result.c02_marketplaceName === '') { return true } //pour filter un result vide, on arrete ici
		if ( (!result.c09_seller || result.c09_seller === '') && (typeof result.c04_asin === 'string' && result.c04_asin !== '') && sellerASIN && sellerASIN.get(result.c04_asin) ){
			result.c09_seller = sellerASIN.get(result.c04_asin)
		}
	...
	let fullResultFieldsDeleted = utils.deleteFields(fullResult,['postid','_id','errorInfo'])
